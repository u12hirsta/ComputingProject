class SliderHolder {
  ...
  float start;
  Button menu = new Button(new PVector(start+width*0.05, height*0.025), new PVector(width*0.1, height*0.05), "Menu");
  SliderHolder(float start) {
    this.start = start;
    for (int i = 0; i <noButts; i++) {
      change[i] = new Button(new PVector((start+(0.2*i)), height*0.03), new PVector(height*0.05, height*0.05), "");
    }
    for (int i = 0; i <increaseDecreaseBtn; i++) {
      changeNoBacts[i] = new Button(new PVector(start*(0.2+(0.2*i)), height*0.025), new PVector(height*0.05, height*0.05), i==0?"-":"+");
    }
